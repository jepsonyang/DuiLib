...
#define MSGDEF(x) if(uMsg==x) return _T(#x)
	...
	HWND CWindowWnd::Create(HWND hwndParent, LPCTSTR pstrTitle, DWORD dwStyle, DWORD dwExStyle, int x, int y, int cx, int cy, HMENU hMenu)
	{
		if (GetSuperClassName() != NULL && !_RegisterSuperclass()) return NULL;
		if (GetSuperClassName() == NULL && !_RegisterWindowClass()) return NULL;
		m_hWnd = ::CreateWindowEx(dwExStyle, GetWindowClassName(), pstrTitle, dwStyle, x, y, cx, cy, hwndParent, hMenu, CPaintManagerUI::GetInstance(), this);
		ASSERT(m_hWnd != NULL);
		return m_hWnd;
	}

	HWND CWindowWnd::Create(HWND hwndParent, LPCTSTR pstrTitle, DWORD dwStyle, DWORD dwExStyle, const RECT rc, HMENU hMenu)
	{
		return Create(hwndParent, pstrTitle, dwStyle, dwExStyle, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, hMenu);
	}

	HWND CWindowWnd::CreateDuiWindow(HWND hwndParent, LPCTSTR pstrTitle, DWORD dwStyle /*= 0*/, DWORD dwExStyle /*= 0*/)
	{
		return Create(hwndParent, pstrTitle, dwStyle, dwExStyle, 0, 0, 0, 0, NULL);
	}
	...
	HWND CWindowWnd::Subclass(HWND hWnd)
	{
		...
		ASSERT(m_hWnd == NULL);
		m_OldWndProc = SubclassWindow(hWnd, _WndProc);
	...
	void CWindowWnd::CenterWindow()
	{
		...
		if (hWndCenter == NULL || IsIconic(hWndCenter)) rcCenter = rcArea;
	...
	LRESULT CALLBACK CWindowWnd::_ControlProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		CWindowWnd* pThis = NULL;
		if( uMsg == WM_NCCREATE )
		{
